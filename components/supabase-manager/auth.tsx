...
function ProviderSettingsView({
  ...
  projectRef: string
  schema: z.ZodObject<Record<string, z.ZodTypeAny>> | z.ZodType<unknown>
  title: string
  ...
  const actualSchema = 'shape' in schema ? schema : schema as z.ZodObject<Record<string, z.ZodTypeAny>>
...
export function AuthManager({ projectRef }: { projectRef: string }) {
  ...
  const handleProviderClick = useCallback(
    (provider: { name: string; schema: z.ZodObject<Record<string, z.ZodTypeAny>> | z.ZodType<unknown> }) => {
      push({
    ...
    return (
      <div className="mx-6 lg:mx-8 mt-8">
        <Alert variant="destructive">
          ...
          <AlertTitle>加载认证设置时出错</AlertTitle>
          <AlertDescription>
            加载您的身份验证配置时出现问题。
          </AlertDescription>
        </Alert>
        ...
        <div className="border rounded-md overflow-hidden bg-background">
          {providers.map((provider) => (
            <Button
              ...
              key={provider.name}
              className="rounded-none justify-start flex-row text-left h-auto px-8 py-2 w-full gap-4 border-b last:border-b-0"
            >
              {provider.icon}
