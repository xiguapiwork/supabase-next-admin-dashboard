...
export function SuggestionsManager({ projectRef }: { projectRef: string }) {
  ...
  return (
    <div className="p-6 pt-4 lg:p-12 lg:pt-12 max-w-3xl mx-auto">
      <h2 className="text-base lg:text-xl font-semibold mb-1">建议</h2>
      ...
      {error && (
        <Alert variant="destructive" className="mt-8">
          ...
          <AlertTitle>获取建议时出错</AlertTitle>
          <AlertDescription>
            {(error as Error)?.message || '发生了意外错误。请重试。'}
          </AlertDescription>
        </Alert>
      )}
      {suggestions && (
        <div>
          {sortedSuggestions.length > 0 ? (
            <div>
              {sortedSuggestions.map((suggestion: { cache_key: string; title: string; level: 'ERROR' | 'WARN' | 'INFO'; type?: string; description: string; detail: string }) => (
                <div
