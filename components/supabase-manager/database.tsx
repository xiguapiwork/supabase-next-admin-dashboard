...
function generateZodSchema(table: TableInfo): z.ZodObject<Record<string, ZodTypeAny>> {
...
const getPrimaryKeys = (table: TableInfo): string[] => {
  if (!table || !table.primary_keys) {
    return []
  }
  return table.primary_keys.map((pk: PrimaryKey) => pk.name)
}
...
function TableRecordsView({ projectRef, table }: { projectRef: string; table: TableInfo }) {
    ...
    <div className="p-6 pt-4 lg:p-8 lg:pt-8">
      <div className="flex items-center justify-between mb-6 gap-6">
        <div className="flex-1">
          <h1 className="text-base lg:text-xl font-semibold">数据库</h1>
      ...
      {isError && (
        <Alert variant="destructive" className="mt-8">
          ...
          <AlertTitle>加载表时出错</AlertTitle>
          <AlertDescription>加载您的数据库表时出现问题。</AlertDescription>
        </Alert>
      )}
