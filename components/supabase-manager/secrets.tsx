...
export function SecretsManager({ projectRef }: { projectRef: string }) {
  ...
  const handleDeleteSecret = (secretName: string) => {
    if (window.confirm(`您确定要删除密钥 "${secretName}" 吗？`)) {
      deleteSecrets({
        projectRef,
  ...
  return (
    <div className="w-full max-w-3xl mx-auto p-6 pt-4 lg:p-12 lg:pt-12">
      <div className="mb-16">
        <h2 className="text-base lg:text-xl font-semibold mb-1">添加新密钥</h2>
        <p className="text-muted-foreground mb-6 text-sm lg:text-base">
          添加您的应用程序安全运行所需的敏感信息，如API密钥。
        </p>
        <Form {...form}>
            ...
            {fields.map((field, index) => (
              <div key={field.id} className="flex items-start space-x-2">
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem className="flex-1">
                      {index === 0 && <FormLabel className="mb-4">名称</FormLabel>}
                      <FormControl>
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem className="flex-1">
                      {index === 0 && <FormLabel className="mb-4">值</FormLabel>}
                      <FormControl>
                ...
                />
                <div className="w-8">
                  {index === 0 && <FormLabel className="opacity-0 mb-4">移除</FormLabel>}
                  <Button
                    type="button"
                    variant="outline"
                    size="icon"
      ...
      <div>
        <h2 className="font-semibold mb-4 lg:text-lg">现有密钥</h2>
        {secrets && secrets.length > 0 ? (
              ...
              <div>
                ...
                {secret.updated_at && (
                  <p className="text-xs text-muted-foreground mt-1">
                    最后更新：{new Date(secret.updated_at).toLocaleString()}
                  </p>
                )}
              </div>
              <Button
          ...
          <Alert>
            ...
            <AlertTitle>未找到密钥</AlertTitle>
            <AlertDescription>此项目未找到任何密钥。</AlertDescription>
          </Alert>
        )}
      </div>
    </div>
