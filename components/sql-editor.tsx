...
export function SqlEditor({
  ...
  const handleRunNaturalLanguageQuery = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.message || '生成SQL失败')
      }
      ...
      console.error(error)
      setAiError(error instanceof Error ? error.message : '生成SQL时发生错误')
    } finally {
  ...
  const serverErrorMessage = (error as Error)?.message || ''
                    ...
                    <div className="grid gap-2">
                      <div className="flex items-center space-x-2">
                        <div className="flex-1">
                          <Label className="flex-1 mb-2 block">显示图表</Label>
                      ...
                      {isChartVisible && (
                        <div className="mt-2">
                          ...
                          <div className="grid grid-cols-3 items-center gap-4 mb-2">
                            <Label htmlFor="y-axis">Y轴</Label>
                            <Select
                              onValueChange={setYAxisColumn}
                              defaultValue={yAxisColumn || undefined}
                            >
                              <SelectTrigger className="col-span-2 h-8">
                                <SelectValue placeholder="例如：显示最近7天注册的所有用户" />
                              </SelectTrigger>
                              <SelectContent>
                                {columns.map((col) => (
                                  <SelectItem key={col} value={col}>
